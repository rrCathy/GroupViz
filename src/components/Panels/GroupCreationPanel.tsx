...
export function GroupCreationPanel() {
  ...
  return (
    <AccordionSection
      ...
      icon="⊕"
      defaultOpen={false}
      badge={currentGroup ? <span dangerouslySetInnerHTML={{ __html: renderTex(currentGroup.symbol) }} /> : undefined}
    >
      {/* Creation mode toggle */}
      <div className="creation-mode-toggle">
        <button
          className={`toggle-btn ${creationMode === 'type' ? 'active' : ''}`}
          onClick={() => { setCreationMode('type'); setSelectedOrder(null) }}
        >
          {t('panel.createByType')}
        </button>
        <button
          className={`toggle-btn ${creationMode === 'order' ? 'active' : ''}`}
          onClick={() => setCreationMode('order')}
        >
          {t('panel.createByOrder')}
        </button>
      </div>
        ...
        <>
          <div className="order-list">
            {availableOrders.map(order => (
              <button
                key={order}
                className={`order-btn ${selectedOrder === order ? 'active' : ''}`}

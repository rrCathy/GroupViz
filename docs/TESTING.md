...
## 3. 测试文件清单（src/__tests__，37 文件 / 763 tests）
...
| semidirectDecompositions.test.ts | 23 | 半直积分解（semidirectDecompositions.ts）：findAutoByMap（命中/不命中/空）、verifyPhiHomomorphism（C4⋊C2 反转≅D4、C2⋊C2 平凡 φ、错阶 auto 拒绝、缺失回退 identity）、buildPhiFromGroup（round-trip/非半直积 null）、minimalGenerators（V4⊂A4 2 生成元、{e}→[]、全群）、buildSubgroupGroup（S3 换位 order2、C6 偶数子群 order3）、findSemidirectDecompositions（S3 3 候选全 verified、D4 8 候选、A4 4 候选 Frobenius、S4 ≥9 双型、C6 2 候选、Q8 []、D12 19 候选全 verified、S5 守卫 []、C7 []） |

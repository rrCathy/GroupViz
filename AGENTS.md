...
| [docs/TESTING.md](docs/TESTING.md) | 测试体系：~95 文件 / ~2000 tests（量级口径，精确数以 `npm run test` 实时输出为准；node+dom 双项目）、Playwright E2E、vitest 配置、覆盖率（core/utils/context/components 四层分层阈值）、测试约定、**法则型性质测试（元素→数学对象映射的 11 条结构法则 oracle）** |
...
| [docs/ROADMAP.md](docs/ROADMAP.md) | 路线图（**只列未做事项/边界决策**）：FGVE 引擎化进行中——**阶段 2 视图 props 化已收官（11/13）**，余 tree/prestable 两个无限群方向视图移交拓展包轨道；其余待办 = action 窗口 sylow/coset、阶段 3 的 CI 消费冒烟接入 / exports 子路径 / descriptor round-trip 载群（双包 @groupviz/core+react **v2.0.0 已发布 npm**，**v2.1.0 消费端卡点优化** = 元素引用/`useSceneState`/`theme`/阈值/`API.md`，2026-09-10）；**§2.7 视图控制层（VCL）进行中**（Phase 0 三件地基开工中）；中期特征标表 / GAP 后端完善；远期 GVL 教学实验室 |
...
| [docs/PLAN_VIEW_CONTROL_LAYER.md](docs/PLAN_VIEW_CONTROL_LAYER.md) | **视图控制层（VCL）规划 + 施工**：§1 定位与边界（三级架构 + 四条边界：主画布不接 / 2D 面填充否决 / 不按群阶特判 / 教学逻辑归 GVL）· §2 四子系统（ViewParams·Decorations·ControlUI·FigurePreset + 已落地字段清单）· **§3 能力清单（唯一真源）**：已交付 15 项 / 本批 3 件 / 池中 Tier A·B·C / 否决 · §4 Phase 0 施工方案（分刀 DEC-1→DEC-2→UI-1→FP-1、接口草案、风险）· §5 决策记录 · §6 参考实现（S₅ 字长球单文件 HTML）吸收台账。**已完成历史一律查 CHANGELOG，本文不复述** |
| [docs/PLAN_WINDOW_FRAMEWORK.md](docs/PLAN_WINDOW_FRAMEWORK.md) | **视图窗口框架融合方案（2026-09-24 立项，方案待批）**：现状三套壳并存（应用老式壳 `FloatingViewWindow` / FGVE 受控内核 `ViewWindow` / 包消费端壳 `SceneWindow`）能力对照 + 分界线（状态归属与视图分发）+ 目标形态（`WindowFrame` + 两种宿主适配器）+ 三个必须先拍板的语义取舍（选中/生成元是否联动）+ 分刀 W-1~W-6 与验收物 + 5 条风险 |
| [docs/PLAN_EXTENSION_PACKS.md](docs/PLAN_EXTENSION_PACKS.md) | **拓展包规划（2026-09-16 立项，未开始）**：拓展包与引擎双包的关系模型（peerDeps 下游消费者 + Group 开放接口 + 三形态边界）+ 三个包分述（@groupviz/symmetric-families 对称群族与点群 / @groupviz/rep-theory 表示论深化 / @groupviz/galois 伽罗瓦对应）+ 性能对照 + 开发顺序 + 引擎侧待开孔清单 |
...
GroupViz/
├── src/
│   ├── __tests__/              # ~95 个测试文件（~2000 tests，量级口径；node+dom 双项目），见 docs/TESTING.md
│   ├── components/
...
- ✅ 13 种视图模式：set / cayley / cycle / table / 3d / symmetry / sublattice / homomorphism / cosetstrip / action / sylow / tree / prestable。**其中 11 个已完成 props 化入包**（`@groupviz/react` 收录 11 个视图 Scene，2026-09-16 sylow 收尾）；tree / prestable 两个群展示专用视图（入口在左侧「群展示」面板底部，不在视图面板卡片中）与**无限群**方向绑定、当前无消费需求，**不再 props 化**，移交拓展包轨道（见 [docs/PLAN_EXTENSION_PACKS.md](docs/PLAN_EXTENSION_PACKS.md) §9）。**视图之外另有一个附属窗口功能入包**：自同构作用预览 `AutomorphismScene`（2026-09-19，第 12 个 Scene）——形态为「内容内核 + 宿主 `SceneWindow` 嵌套预览窗」，主应用浮层与包消费端共用同一份实现（见 [docs/API.md](docs/API.md) §4.12）。**附属组件再添一个**：商群悬浮窗 `QuotientSubgroupInset`（2026-09-21）——`SetView`/`CayleyView` 内置出窗（`quotientInsetTitle` prop），门面亦可直用；core 侧 `computeQuotientGroup` / `quotientInsetGeometry` 同步公开（见 [docs/API.md](docs/API.md) §4.13）
...
- ✅ 广义 Cayley 图：右乘/左乘切换、任意元素作用边、14 种 2D 形状（含 rewiring、直积 cylinder/torus）、20 种 3D 形状模板（按群性质自动分配；含 S₄ 星形对换 (12)(13)(14) 的**环面全六边形镶嵌** `torusHex`——24 顶点 36 边 12 六边形、周期格 60° 菱形，见 docs/CAYLEY.md §6）
...
- ✅ **字长球形状（`wordLengthSphere`，2026-09-12，已入引擎公共面）**：S₄/S₅ 相邻对换生成集 (12)(23)(34)(45) 的**字长分层实心球**（不是球壳、不是投影）——S₄ 借截角八面体骨架（= 凯莱图）周向角重排到字长层（极点 120° 三等分星形、全顶点贴球面）；S₅ **纬度分层**（y_l = R·cos(πl/10) 极点 ±R、层截面 r_l = R·sin(πl/10)）+ **正根胞格向量初值**（A₄ 十个正根手工投影：四个单根恰落 +x/+y/−x/−y ⇒ 四个生成元 = 四方位瓣）+ **边距松弛 320 轮**（同层斥力 + 边-边斥力（AABB 预筛 + 3 采样点线段距离）+ 角度弱锚定 + y 弹簧；硬约束纬度带 ±0.32 层距、水平半径 ≤ min(r_l, √(R²−y²))；收尾 r > 0.9·r_l 精确吸附球面）⇒ 实心球（外圈贴壳 + 内部有点）；相邻对换使字长恒 ±1 ⇒ **240 条边全部跨相邻层、层内无边**；渲染配套：**半透明球壳**（半径 = 最外层节点球面、`depthWrite:false` 不遮挡球内节点）、字长色阶（`wordLengthColor` 205°→330°）、深度雾；**引擎导出**：`@groupviz/core` 提供 `wordLengthSphereActions`（**结构判定**的相邻对换作用边，S₄ 3 / S₅ 4 条，非 S₄/S₅ 或结构不符返回 null）、`wordLengthSphereLayout3D`、`wordLengthOf`、`wordLengthColor`、`findAdjacentTranspositionGenerators`；应用层（`context/cayleyActions` 的 `getSpecialCayleyActions`/`addAllCayleyActionsHelper`、`ViewPanel.canonical3DEdgeIds`）**全部委托核心同一实现**（硬编码元素 id 清零），`docs/API.md` §4.9/§7 给消费示例，发布门禁 `publish:smoke` 增字长球断言（4 条作用边 + 120 点布局 + 色阶）
...
- ✅ 群展示（Presentation）系统：任意展示 ⟨a,b | f(a,b), …⟩ 输入创建有限群（Todd–Coxeter 陪集枚举，TC_MAX_COSETS=3000 守卫判有限/无限/溢出），乘法表构建 + `presentationOf` 自动识别标准展示（Cₙ/Dₙ/Sₙ Coxeter/Aₙ/V₄/Q₈，直积/半直积/商群/Aut 走通用关系发现器，PRESENTATION_MAX_ORDER=240）；**发现器极小化展示**（发现器词枚举先 `simplifyWord` + `isRedundantRelator` 前缀过滤，再 `canonicalCyclicForm` 循环旋转/逆折叠（关系 w=e 蕴含任意循环旋转与逆也是关系，QD16 的 868 条共轭冗余词折叠为少数规范词）+ 贪心移除（保护 K 条最短关系即生成元阶关系如 a⁴/b²，其余长词逐个 TC 验证移除，1.5s 预算守卫）；**`presentationOf` 对 QD16 特判返回标准展示 ⟨a⁸, b², bab=a³⟩**（按元素阶定位阶 8 生成元 a——GAP 生成元 g1 阶 4 只能导出非标准 ⟨a⁴, b², (ba)⁴⟩）+ **发现器结果缓存**（symbol|order|生成元数键，群信息栏/树视图反复调用不再重算约 1.6s）；关系支持 **Unicode 上标（a²、a⁻¹，`normalizeSuperscripts`）与 f1=f2 等式（`ab=ba` → `aba⁻¹b⁻¹`，e=f 取另一侧）**；群信息栏展示 TeX 行；左侧「群展示」面板（**两种创建方式**：直接创建 = 完整展示文本 + 持久保存/草稿自动保存 localStorage；**可视化创建** = 1/2/3 生成元模板 ⟨a|⟩/⟨a,b|⟩/⟨a,b,c|⟩ + 逐步输入单条关系（严格 f=e 或 f1=f2 校验，`parseRelationEquation`）+ 已添加关系列表实时构建校验（|G|、≅、无限判定）+ 「结束并创建群」；创建成功设置 **`activePresentationGroup` 独立状态**（不替换左侧当前群，`?? currentGroup` 回退）+ 自动切树视图；「✕ 清空当前群（回到模板树）」）；**tree视图**（退化树 = 商群凯莱图的 BFS 生成树：实线=首次到达边（按生成元着色）；粘合边不绘制，仅顶部 bar 金色「粘合边 ×N」计数，直观呈现「关系 = 砍树」；布局按群结构规则化——1 生成元直线、2 生成元交换格 → 正方形网格（不衰减，幂关系下指数按 mod 折叠，a²,b³,ab=ba → 2×3 网格）、非交换/自由积 → 谢尔宾斯基十字（层距减半防遮挡）、3 生成元 → **3D**（Three.js 立方体方向）；路径状树 0.7/稠密树 0.5 层距衰减 `stepForDepth(depth, ratio)`；展示生成元 ≠ 群生成元时用 `pres.generatorElements` 求值（`genElsOverride`，修复 S₄ Coxeter 3 生成元）；点击节点显示词、缩放/平移/双击复位；无群时展示自由模板树）、**展示乘法表视图**（行/列=群元素，顶部静态展示式 bar，order>36 自动采样）；核心算法 `src/core/algebra/presentations.ts` + `src/core/algebra/cayleyTree.ts`（见 docs/PRESENTATION.md）
  - ✅ 测试体系：~95 文件 / ~2000 tests 全绿（`npm run test`，node+dom 双项目；**量级口径**——精确数以实时输出为准，构成口径见 [docs/TESTING.md](docs/TESTING.md)）；覆盖率**四层分层阈值**（per-glob：core/utils ≥ 85/70，context/components 防倒退线，见 vitest.config.ts `coverage.thresholds`）；vitest `test.projects` 双项目拆分（node=*.test.ts、dom=*.component.test.tsx/*.integration.test.tsx + setup.ts jest-dom/ResizeObserver/matchMedia stub）；**E2E Playwright**（`npm run test:e2e`）：e2e/ 7 spec（欢迎页入口、三栏布局+pageerror==[]、9 视图卡遍历、建群 D₄+直积模式、session reload 恢复+主题持久化、3 截图视觉回归基线 maxDiffPixelRatio 0.02；chromium 单浏览器 workers:1 locale zh-CN webServer strictPort）；`npm run typecheck`（tsc -b）；**GAP 表群惰性审计**（tableGroups.audit.test.ts 扫全部 66 表群：默认形状/全部可用形状/圆形布局/生成元边端点半直积 meta，杜绝布局静默回退回归）（`npm run test:coverage`）
- ✅ **视图控制层（VCL）**：在「计算层 + 视图渲染层」之上叠一层插图编排（规划 + 施工 [docs/PLAN_VIEW_CONTROL_LAYER.md](docs/PLAN_VIEW_CONTROL_LAYER.md)：三级架构 + ViewParams/Decorations/ControlUI/FigurePreset 四子系统，**能力清单唯一真源在其 §3、施工方案在其 §4**）。**边界（2026-09-24 用户定）**：主画布不接 VCL（主画布 = 新视图/新形状的先行试验田，VCL 只在 FGVE 窗口 + 包 props 生效）、2D 面/陪集填充否决（2D 已够乱，陪集语义留给商群悬浮窗 + 3D `faceFill`）。已落地 **2D 凯莱图**：边曲率 `edgeCurvature`（0 = 笔直，平行边按作用序号左右分摊）、逐生成元边长 `actions[].lengthScale`（`core.relaxEdgeLengths` 通用后处理，全 1 逐位不变）、路径高亮 `pathHighlight`（元素序列 / 生成元单词，方向敏感，可选动画/序号；**缺省 `dimOthers` 淡化其余边**——"只显示路径"，序号 **hover 该节点时才显示**）、动态力导向 `forceDirected`（**开关**而非新形状 + `core.createCayleyForceSim`；**Obsidian 式局部性**：位移按热度缩放（`x += vx × alpha`，FR 标准）+ 拖拽热度 0.2（1 跳邻居 ~20–30% 粘性跟随、其余 2–7%）+ 松手 0.25（轻微弹性收尾）⇒ 拖动一个节点只影响近旁（路径上撞到的节点由 `minSeparation` 硬约束推开）；**参数（link/rep/gravity/damping/stiffness）走 `sim.setOptions` 就地更新**（保留位置与速度 + 温和升温 0.55）——滑杆是平滑可感的调整（实测 20–31px）而非"重建 sim + 全热度重收敛"的大变样；整体重排走面板「⟳ Re-settle」；**拖拽 1:1 跟手**——拖拽起点必须快照 `{x,y}`（`pin()` 会原地改写 `positions` 对象）；**4px 拖拽阈值**：点击不 pin/不升温；`kSpring` 0.28 + 斥力 3.5×理想间距截断、`gravK` 0.006、`force.stiffness` 0.4–3 可调）；**3D 凯莱图**：逐生成元边长 `Cayley3DScene.actions[].lengthScale`（`core.relaxEdgeLengths3D` 三维松弛，斥力按基础布局平均边长自适应、无边界钳制）与路径高亮 `Cayley3DScene.pathHighlight`（复用 `core.resolveCayleyPath`；drei `Line` 线段 + 节点环球壳 + 序号 `Html`；**同 `dimOthers` 淡化非路径边**（材质 `needsUpdate` 保证更新生效）；受控悬停 `Cayley3DScene.hoveredElementId` 与 2D 对称）。控制 UI：FGVE 窗口 ⚙ 面板（2D/3D 各一套，`CayleyPathEditor` 双模式编辑器 + 逐 action len 滑杆 + Dim others 开关 + Rigidity 滑杆）与 TestPage1 引擎消费卡片（props 注入 + 页头 4 键「VCL 示例」一键预设：S₃ 笔直 / A₄ 截角四面体逐生成元倍率 / S₄ 字长球哈密顿路径 / D₇ 2D 力导向；两张凯莱卡片的形状下拉按 `getAvailableShapes3D`·`getAvailableShapesForView` 动态枚举）；发布门禁 `publish:smoke` 含 `relaxEdgeLengths3D` 全 1 原样返回 + 2× 拉长断言与 `Cayley3DSceneProps` 类型面断言。**零配置安全**：所有新 props 可选，未传时外观与既有完全一致

### 里程碑速览(截至 v2.1.0,2026-09-10)

- 🔄 **当前进行**:FGVE 引擎化收尾——**阶段 2 视图 props 化已收官(11/13,2026-09-16 sylow 入包)**;余 action 窗口 sylow/coset、CI 消费冒烟接入、exports 子路径;tree/prestable 两个无限群方向视图移交拓展包轨道(见 [docs/PLAN_EXTENSION_PACKS.md](docs/PLAN_EXTENSION_PACKS.md) §9);**VCL 由挂账转进行中(2026-09-24,ROADMAP §2.7)**——三批已交付(边几何/路径高亮/语义装饰 F·E 组 + 3D B 组),**下一批 = Phase 0 三件地基**(`Decorations` 注释 / `FigurePreset` 插图预设 / 注册表驱动 ⚙ 面板,施工方案见 [docs/PLAN_VIEW_CONTROL_LAYER.md](docs/PLAN_VIEW_CONTROL_LAYER.md) §4);两条边界:**主画布不接 VCL**(主画布 = 新视图/新形状的先行试验田)、**2D 面/陪集填充否决**(2D 已够乱) —— 见 [docs/ROADMAP.md](docs/ROADMAP.md) §2.7。
...
- **新增文件后必须补跑全量 `npm run lint`**：改动文件级 lint 会漏掉跨文件规则（`react-hooks/*` 全靠 React Compiler 的类型推断，规则触发点常在**调用方**而非定义方）
- **公共 hook 的返回体不得在顶层放 ref 载体**：`react-hooks/refs` 的判定粒度是整个返回对象——顶层出现 `RefObject` / 写 ref 的回调 / 裸 `useState` setter，宿主任何 `s.xxx` 读取（含 spread、`{s.hoverBubble}`）都会报 *Cannot access refs during render*。正确做法：把回调 ref **嵌套**进 `hostProps.ref` 之类的子对象，DOM 节点另开 `getHostElement()`；解构与元组返回也不触发

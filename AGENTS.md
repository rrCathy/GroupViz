...
| [docs/CAYLEY.md](docs/CAYLEY.md) | Cayley 图系统：边计算、2D/3D 渲染、18 种 3D 形状模板、14 种 2D 形状布局 |
...
| [docs/TESTING.md](docs/TESTING.md) | 测试体系：75 文件 1642 tests（node+dom 双项目）、Playwright E2E 13 tests、vitest 配置、覆盖率、测试约定 |
| [docs/ACTIONS.md](docs/ACTIONS.md) | 群作用系统：共轭/正则/陪集/自定义/Sylow 五来源、同态校验、轨道/稳定化子/OST、Burnside 自检、轨道视图、几何作用暂缓记录 |
| [docs/ROADMAP.md](docs/ROADMAP.md) | 路线图：近期功能夯实期已收官（E1 gappy 已关闭，换道直连 GAP）+ FGVE 前置工程准备收官（P0 v1.15.1 / 测试体系 v1.16.0，2026-08-24 终验四绿） → **中期 FGVE 引擎化（进行中）**：GroupDescriptor v1 序列化 + 视图 props 化四批重构 + `@groupviz/core`/`@groupviz/react` 双包可构建（2026-08-25 定案，见 ROADMAP §2.8） → 远期 GVL 教学实验室（只列未做事项） |
...
GroupViz/
├── src/
│   ├── __tests__/              # 75 个测试文件（1642 tests，node+dom 双项目），见 docs/TESTING.md
│   ├── components/
...
- ✅ 循环图（cycle graph）`cycleGraphLayout`（GE 忠实复刻 uniteParts+bestPowerRelativeTo）：单位元 e 固定在原点永不重放、极大循环=花瓣圆弧（第 k 非单位元 gᵏ 落圆心 (0,R) 半径 R 弧上 (-R·cosθ,R(1+sinθ))，θ=2π(k/n−0.25)）；循环按共享非单位元聚成 part（uniteParts 并查集，并入循环经 bestPowerRelativeTo 重轮换使共享元素同弧索引对齐）；part 按循环长度和比例分配角度弧（单 part 半圆弧、最大 part>1/2 封顶、最大 part 弧心朝下）；part 内多循环 gravity=ringNum/part.length 拉向弧心；共享元素只由首个循环放置后续引用（蝴蝶 Z₂×Z₄、SL(2,3) 7 循环共点 −I 太阳放射）；不在 ≥3 阶循环的 2 阶元素=叶柄线段；风车式：S3 型只共享 e 的群（S3/Dn/A4/A5/V4/纯直积）均匀绕 e 一圈（360° 等分扇区 g=0，不朝下簇拥）；共享判定排除 e（防恒并）；C₅×S₃ 等复杂共享为 GE 固有少量边交叉（无回退）；**循环图浮动窗口（ViewWindow）类似凯莱图**：默认不显示元素标签（showLabels=false，mirror cayley；循环参数面板已移除 Show labels 复选框）；点击元素→高亮显示它所在极大循环（CYCLE_COLORS 彩色填充 fillOpacity 0.18 + strokeWidth 4 + ⟨g⟩≅Z_n 标注）
...
- ✅ 群展示（Presentation）系统：任意展示 ⟨a,b | f(a,b), …⟩ 输入创建有限群（Todd–Coxeter 陪集枚举，TC_MAX_COSETS=3000 守卫判有限/无限/溢出），乘法表构建 + `presentationOf` 自动识别标准展示（Cₙ/Dₙ/Sₙ Coxeter/Aₙ/V₄/Q₈，直积/半直积/商群/Aut 走通用关系发现器，PRESENTATION_MAX_ORDER=240）；**发现器极小化展示**（发现器词枚举先 `simplifyWord` + `isRedundantRelator` 前缀过滤，再 `canonicalCyclicForm` 循环旋转/逆折叠（关系 w=e 蕴含任意循环旋转与逆也是关系，QD16 的 868 条共轭冗余词折叠为少数规范词）+ 贪心移除（保护 K 条最短关系即生成元阶关系如 a⁴/b²，其余长词逐个 TC 验证移除，1.5s 预算守卫）；**`presentationOf` 对 QD16 特判返回标准展示 ⟨a⁸, b², bab=a³⟩**（按元素阶定位阶 8 生成元 a——GAP 生成元 g1 阶 4 只能导出非标准 ⟨a⁴, b², (ba)⁴⟩）+ **发现器结果缓存**（symbol|order|生成元数键，群信息栏/树视图反复调用不再重算约 1.6s）；关系支持 **Unicode 上标（a²、a⁻¹，`normalizeSuperscripts`）与 f1=f2 等式（`ab=ba` → `aba⁻¹b⁻¹`，e=f 取另一侧）**；群信息栏展示 TeX 行；左侧「群展示」面板（**两种创建方式**：直接创建 = 完整展示文本 + 持久保存/草稿自动保存 localStorage；**可视化创建** = 1/2/3 生成元模板 ⟨a|⟩/⟨a,b|⟩/⟨a,b,c|⟩ + 逐步输入单条关系（严格 f=e 或 f1=f2 校验，`parseRelationEquation`）+ 已添加关系列表实时构建校验（|G|、≅、无限判定）+ 「结束并创建群」；创建成功设置 **`activePresentationGroup` 独立状态**（不替换左侧当前群，`?? currentGroup` 回退）+ 自动切树视图；「✕ 清空当前群（回到模板树）」）；**tree视图**（退化树 = 商群凯莱图的 BFS 生成树：实线=首次到达边（按生成元着色）；粘合边不绘制，仅顶部 bar 金色「粘合边 ×N」计数，直观呈现「关系 = 砍树」；布局按群结构规则化——1 生成元直线、2 生成元交换格 → 正方形网格（不衰减，幂关系下指数按 mod 折叠，a²,b³,ab=ba → 2×3 网格）、非交换/自由积 → 谢尔宾斯基十字（层距减半防遮挡）、3 生成元 → **3D**（Three.js 立方体方向）；路径状树 0.7/稠密树 0.5 层距衰减 `stepForDepth(depth, ratio)`；展示生成元 ≠ 群生成元时用 `pres.generatorElements` 求值（`genElsOverride`，修复 S₄ Coxeter 3 生成元）；点击节点显示词、缩放/平移/双击复位；无群时展示自由模板树）、**展示乘法表视图**（行/列=群元素，顶部静态展示式 bar，order>36 自动采样）；核心算法 `src/core/algebra/presentations.ts` + `src/core/algebra/cayleyTree.ts`（见 docs/PRESENTATION.md）
 - ✅ 测试体系：75 文件 1640 tests 全绿（`npm run test`，node 56 文件/1488 + **dom 双项目 19 文件/152**：组件测试 Tex 12/AccordionSection 8/TabBar 8/CayleyView 13/Cayley3DView 9/CayleyWindowParams 8/Cayley3DWindowParams 8/ViewWindowParams 9/CycleTableWindowParams 9/SublatticeView 12/SublatticeWindowParams 7/HomomorphismWindowParams 8/ActionWindowParams 7 + 集成测试 BasicGroupPanel 5/Workspace 7/SvgSnapshot 4/CycleView.integration 1；node 侧含 s4toS3.test.ts：S₄→S₃ 满同态划分作用构造验证），总覆盖率 89.16% stmts / 78.57% branches / 92.31% funcs / 91.96% lines（CI 阈值：stmts/lines/funcs ≥ 85、branches ≥ 70）；vitest `test.projects` 双项目拆分（node=*.test.ts、dom=*.component.test.tsx/*.integration.test.tsx + setup.ts jest-dom/ResizeObserver/matchMedia stub）；**E2E Playwright**（`npm run test:e2e`）：e2e/ 6 spec 13 tests（欢迎页入口、三栏布局+pageerror==[]、9 视图卡遍历、建群 D₄+直积模式、session reload 恢复+主题持久化、3 截图视觉回归基线 maxDiffPixelRatio 0.02；chromium 单浏览器 workers:1 locale zh-CN webServer strictPort）；`npm run typecheck`（tsc -b）；**GAP 表群惰性审计**（tableGroups.audit.test.ts，279 tests 扫全部 66 表群：默认形状/全部可用形状/圆形布局/生成元边端点半直积 meta，杜绝布局静默回退回归）（`npm run test:coverage`）
...
- ✅ **FGVE 阶段 2 批次一（v1.17.0，2026-08-28 起 set/cayley/cycle/table 视图 props 化 + 受控 ViewWindow）**：**GroupDescriptor v1 序列化协议**（src/core/descriptor.ts：zod schema GroupDescriptorSchemaV1{schemaVersion/symbol/name/order/elements[{id,label,order}]/multiply/properties/construction/source[local|import|gap|product]} + serializeDescriptor/deserializeDescriptor/descriptorToSymbol）+ **ViewConfig JSON 化**（src/core/types/viewConfig.ts：ViewWindowConfig{locked,zoomLocked,showInfo,viewportFixed,resizable,showControls,showZoomSlider}、SetViewParams{nodeRadius 8-120,gap 0-100,columns 0-50,showLabels}、CayleyActionParam{elementId,enabled,color}、CayleyViewParams{shape2D,multiplyType,actions,nodeRadius,showLabels}、CycleViewParams{showMaximalCycles/nodeRadius/showLabels/showCycleLabels}、TableViewParams{strategy/cellSize} 全 zod schema，actions 上限 240，core 零 react/localStorage）；CAYLEY_SHAPES_2D 常量 + CayleyShape2D 类型置于 types/view.ts。**受控视图四件套**（props 化 + 对应 *FromContext adapter 壳，主应用零变化）：CayleyView（布局 null→cayleyCircleLayout、normalizeCayleyActions、右/左乘、每实例 cv{n} marker、 rewiring φ-不动点青色高亮、大群 culling、拖拽会话态）、SetView、CycleView（极大循环花瓣布局 cycleGraphLayout、每实例 cyv{n} filter 前缀）、TableView（大群三策略 + onLayoutSize 最小尺寸）。**受控 ViewWindow**（FloatingViewWindow.tsx 新 ViewWindow 724 行起，旧 FloatingViewWindow 保留供主应用多视图）：通用参数面板（View Config 6 复选 + 按视图的 Set/Cayley/Cycle/Table 参数 + ↺ Reset）+ 受控/非受控双模式 + gv-vw- 前缀 versioned 持久化（按视图 schema 校验回退）+ 窗口拖/8 方向缩放 + 内容 Ctrl/Cmd 滚轮缩放（rAF 节流/双击复位）+ 参数面板外置右侧溢出翻左 + VIEWWINDOW_RESET_EVENT/resetAllViewWindows()（utils/resetViewWindows.ts）+ `/?test=1` TestPage 参数矩阵（11 set + 5 cayley + cycle/table/heatmap 窗口）+ `/?test=2` A₄ 博客（TestPage2 深色博客 + 5 个锁定 ViewWindow 插图，FIGURE/LOCKED 两种配置）。当前 61 文件 1510 tests 全绿（node 50/1426 + dom 11/84）+ lint/typecheck/build 通过 + 浏览器实测
- ✅ **FGVE 阶段 2 批次二（v1.18.0，2026-08-31，3D 凯莱图 props 化 + 受控 ViewWindow）**：**Cayley3DView 拆分**（原文件内拆 `Cayley3DScene` 受控内核（含 R3F Canvas 全 props：group/selectedElements/onSelectElement/actions/multiplyType/layout3D/nodeScale/autoRotate/showLabels/locked/subsetHighlights，hover 本地化不再依赖全局 HoverContext，导出桥 `.canvas-viewport` closest 检查保留 → 窗口实例天然不注册）+ `Cayley3DView` context 组装壳导出签名不变，GroupCanvas/旧浮动窗零变化）；**LAYOUTS_3D 常量化**（types/view.ts `LAYOUTS_3D as const` 19 值，Layout3D 类型改派生类型不变，zod `z.enum(LAYOUTS_3D)`）；**Cayley3DViewParams**（viewConfig.ts：layout3D（缺省 getDefaultLayout3D(group)）/multiplyType/actions（复用 CayleyActionParam + normalizeCayleyActions）/nodeScale 0.5-2（缺省 1，节点球 0.42/0.55×scale）/autoRotate（prop 优先 ▶ 按钮本地态兜底）/showLabels（hover/选中 Html 标签）全 zod schema + 持久化 schema 链 `view === '3d'` 分支）；**ViewWindow 3d 分支**（renderContent 渲染 Cayley3DScene 不传 canvasTransform、不接窗口 hover HUD——3D 自带 Html 标签承担 hover 反馈；**三处 3D guard**：wheel handler `view==='3d'` return（Ctrl+滚轮透传 3D 相机 radius 缩放不产生窗口 ct）、onCtMDown return、zoom slider/figure-hint 偏移 3D 隐藏；**参数面板 Cayley 3D View 段**：Layout select（getAvailableShapes3D(group)、value 可用性回退 getDefaultLayout3D）、Multiply a·c/c·a、Node size 滑杆 0.5-2.0 ×1.0、Auto rotate、Show labels、Edge actions 勾选+All/None（**All 传 `'3d'` 与当前形状** → S₄/A₅ 多面体 canonical 轴序自动生效）；locked=禁轨道拖拽/滚轮/右键平移保留点击选中+双击复位）；**TestPage 3D 矩阵 4 窗**（默认 cone/S₃ hexagon/D₄ 受控 autoRotate+左乘/C₄ locked+showLabels:false）；**R3F mock 测试先例**（vi.mock @react-three/fiber：Canvas→div 容器、useThree→stub gl/camera/scene、useFrame noop、drei Html→透传 wrapperClass 的 div、i18n/theme 同 mock，three 纯数学真实；happy-dom 下 React 19 对自定义元素对象 props 序列化 '[object Object]' attribute——边几何断言走 cylinderGeometry args[2] 边长 attribute）；测试：dom +17（Cayley3DView 9 / Cayley3DWindowParams 8）→ **63 文件 1527 tests 全绿（node 50/1426 + dom 13/101）** + lint/typecheck/build 通过 + 浏览器实测（4 窗渲染/面板切 cone 相机回正/持久化跨刷新（cone+left+autoRotate）/Ctrl+滚轮不产生窗口缩放/主应用 3D 视图零回归）

- ✅ **FGVE 阶段 2 批次三（v1.19.0，2026-08-31，子群格 props 化 + 受控 ViewWindow + 小窗可读性）**：**核心问题**——名片硬编码 160×72/字号 15px 且 viewBox 有 1000×600 下限，塞进 520×420 窗口时连 6 节点的 S₃ 格都只剩 7px 文字，S₄（30 子群）完全不可读。**四层解法**：① **fit**：新建 `src/core/algebra/latticeLayout.ts`（`computeLatticeLayout` 纯函数，去下限改「内容包围盒 + 每层居中分槽」，slotW/rowH 返回供 LOD 用；`orderLevelsByBarycenter` 交替自上/下扫描排同层序降交叉 + `countLatticeCrossings` 诊断；`latticeFitScale` 适配缩放**钳 ≤1**（只缩小不放大，主画布观感不突变；宿主尺寸未测得（首帧/happy-dom）→ fit=1））；② **LOD 三档自动降级**：按槽位**屏幕宽+高双指标**定档（`latticeLodTier(slotScreenWidth, rowScreenHeight)`：≥150×66 full / ≥56×26 compact / 其余 dots；双指标是必需的——合并后的轨道格窄高，单看宽度会误降），full 保持原名片、compact 屏幕恒定胶囊只留一行（顶层群符号走 KaTeX foreignObject）、dots 屏幕恒定圆点 + 类别色编码（系列色>中心金>Sylow橙>正规紫>平凡灰>顶蓝>普通灰）；compact/dots 几何按 `1/eff` 反算并以槽位封顶，故任何密度下都不重叠；③ **共轭轨道合并**：`subgroupConjugacyOrbits`（**闭包只用生成元**——⟨S⟩=G ⇒ 生成元共轭闭包=全元素共轭轨道，代价 O(Σ|orbit|·|S|·|H|) 使 S₆ 也可用；键沿用 sorted ids join；`size·normalizerOrder=|G|` 轨道-稳定子自检）+ `mergeLatticeByConjugacy`（Hasse 边像→去重→`transitiveReduce`（新导出，先去重再判「删边后仍可达」，避免平行边双双被删）→`levelsByOrderRank`（新导出，与原格同一分档规则）；MERGE_MAX_NODES=2000 守卫超限回退；全单点轨道（阿贝尔群）走恒等快路径），S₃ 6→4、A₄ 10→5、S₄ 30→更少；④ **信息外置**：底部 `lattice-caption` 一行按 hover ?? 选中给出 `|H|=3 ≅ C₃ · [G:H]=4 · 4 个共轭子群 · |N_G(H)|=3 · ⟨(234)⟩ · N₁·导列`，结构符号由新函数 `subgroupStructureSymbol`（母群上就地算子集阶分布+交换性，复用抽出的 `identifyByDistribution`/`candidateTests` + 候选阶分布缓存，O(|H|²) 只在悬停节点上跑）提供；series 面板在窗口里默认收起（showSeriesPanel=false）改由 caption 承载。**受控四件套**：`SublatticeScene` 内核（全 props 可选、缺省在渲染层 `??` 解析、`lattice` 未传则 order≤60 自算、>60 显示 `lattice.backendOnly`）+ 尾部同名组装壳 `SubgroupLatticeView()`（导出签名不变，GroupCanvas/旧浮动窗零改动）+ `SeriesPanel` 纯呈现外提 + ViewWindow 四点接入（ViewParams union/schema 选择器/renderContent/参数面板：Label detail 下拉、Merge conjugates 复选、Card size 0.6–1.6、Series panel）；**svg 必须绝对定位在 flex:1 宿主内**（否则其 viewBox 宽高比以 min-content 高度撑破定高窗口，Chrome 行为）。**顺带修复受控模式既有缺陷**：ViewWindow 渲染与交互此前直读内部 state，而受控时 state 不更新 → 受控窗口停在初始参数；改为 `config = configProp ?? configState`、`viewParams = viewParamsProp ?? viewParamsState` 统一走「有效值」。**TestPage** 补 5 窗矩阵（S₃ auto/S₄ auto dots/A₄ merge/Q₈ 强制 full 对照/D₄ 受控 compact+0.7）并给 `?test=1|2` 补 I18nProvider（此前 t() 回落原始 key）；**e2e 修复**：v1.18.0 起头部有两个 `.theme-toggle`（应用主题 + 视图窗口主题）致 strict mode 冲突，选择器改 `button.theme-toggle[title^="Switch"]`；子群格视觉基线重截（紧凑包围盒 + barycenter）。测试：新增 node `core/latticeLayout.test.ts`(15)/`core/subgroupOrbits.test.ts`(11) + dom `SublatticeView.component.test.tsx`(12)/`SublatticeWindowParams.component.test.tsx`(7) + viewConfig +5 → **67 文件 1574 tests 全绿（node 52/1454 + dom 15/120）** + lint/typecheck/build 通过 + **E2E 13 tests 通过** + 浏览器实测（主画布随窗口增高自动升 full、A₄ 合并 caption 4·3=12 轨道-稳定子吻合、console 零错误）
- ✅ **FGVE 阶段 2 批次四（v1.20.0，2026-09-05，陪集条带 props 化 + 受控 ViewWindow）**：核心问题——主画布走 subsets 状态自包含，受控窗口须自包含（数据由 group 自算），故选 H 子组候选列表=共轭轨道合并（index 升序、默认首候选）、`CosetStripScene` props 化（主画布壳不变，GroupCanvas/老浮动窗零改动；coset 数据=computeCosetData → elementMap/Colors/Highlight 喂 Scene；持久化按 schema 校验、换群失效回退默认 H；锚点 hover 换算世界→视口与 CycleView 同约定）；ViewWindow 四点接入（schema 选择器 / renderContent cosetstrip 分支 / 参数面板 Subgroup H select + Coset type gH/Hg 分段 + Show labels + Subgroup Cayley ring）；窗口缺省隐藏节点标签/隐藏顶部 H 凯莱圈（与 cayley/cycle 窗口一致读元素靠悬停气泡，面板开关可开回主画布观感）；TestPage 补 4 窗矩阵（S₃ 默认 / Q₈ cosetType:right / C₄ 全开对照 / A₄ 受控），TestPage2 A₄ 博客加第 6 节「V₄ ⋊ C₃ 的陪集分解」配图窗口。测试：node `core/cosetStrip.test.ts`(9) + dom `CosetStripWindowParams.component.test.tsx`(7) → 69 文件 1590 tests 全绿 + lint/typecheck/build + E2E 13 passed + 浏览器实测（深/浅色窗口、cosetType/C₂ 切换 2→3 条带、零 console 错误）
- ✅ **FGVE 阶段 2 批次五（v1.21.0，2026-09-06，同态视图 props 化 + 受控 ViewWindow）**：`HomomorphismView`/`FirstIsomorphismAnimation` 拆 `HomomorphismScene`/`FirstIsomorphismAnimationScene`（纯 props：source/target/mapping/result/name/theoremMode/onTheoremModeChange/theoremAnimation/showLabels/onHover，`setTheoremPhase` 回写改 `onPhaseChange`；主画布壳签名不变零改动）；ViewWindow 经 `homomorphism?: Homomorphism` 单 prop 打包双群接入（view==='homomorphism' 时 group 可为 null）+ `HomomorphismViewParams{showLabels?}` schema + 参数面板 Homomorphism View 段 + title/infoText/hoverOrder/persistKey/displayKey 派生 homomorphism 优先 + 视作自管理布局禁用 ct 平移缩放（同 3d/symmetry）；窗口缺省隐藏节点标签靠悬停就地气泡。TestPage 补 3 窗（C₆→C₂ 自然投影 / S₃→S₃ 恒等 / **S₄→S₃ 划分作用满同态**：核 V₄ 双对换红 + Im=S₃ 青 + G/Ker=24/4=6≅Im 第一同构定理可视化）。测试：dom +8（HomomorphismWindowParams）+ node +1（s4toS3 满同态构造验证）→ 73 文件 1623 tests 全绿
- ✅ **FGVE 阶段 2 批次六（v1.22.0，2026-09-06，群作用视图 props 化 + 受控 ViewWindow：conjugation / regular / custom 三来源）**：`ActionView`（1028 行）拆 `ActionScene.tsx` 纯 props 内核（banner + DisplayMode + CustomActionEditor + Stab box 全场景进 Scene；ClusterNode hover 联动仅窗口模式挂载——主画布壳零行为变化）+ `ActionView.tsx` 降级 context 壳；`actions.ts` 新增 arrowListAdd/Bind/Remove/ReplaceGen 箭头列表纯变换（GroupActionContext 与窗口共享单一事实源）；`ActionViewParams{actionKind?/setSize?/arrows?/showLabels?}` + schema（enum 仅三来源，sylow/coset 留待后续）；ViewWindow 接入：窗口数据自算（conjugation/regular 直算 / custom 由已验证 arrows 自算，空 arrows=平凡作用合法，坏值回退 noAction）+ 参数面板 kind 三分段（Conjugation/Translation/Custom）+ custom 编辑流（|X| 输入 + Edit arrows / Complete & verify / Clear / Cancel——编辑态窗口本地不持久化，viewParams 只存已验证结果，博客可注入预验证 arrows）+ 编辑模式禁 ct 拖拽/滚轮/zoom slider + showLabels=false 节点空圈 + chips 区隐藏 + 悬停就地气泡（与 homo/cosetstrip 窗口一致）。TestPage 补 4 窗矩阵（S₃ 共轭 / C₄ 平移 / S₃ custom 预注入 X=3 自然作用 / A₄ 共轭受控）。测试：node `core/actionViewConfig.test.ts`(10) + dom `ActionWindowParams.component.test.tsx`(7) → 75 文件 1640 tests 全绿 + lint/typecheck/build + E2E 13 passed + 浏览器实测（S₃ 共轭轨道/★/hover 气泡/Stab box OST、C₄ 平移、A₄ 切 Translation、custom 预注入重开直显、console 零错误）
- ✅ **FGVE 阶段 2 批次七（v1.23.0，2026-09-07，对称性视图 props 化 + 受控 ViewWindow）**：`SymmetryView.tsx` 拆 `SymmetryViewScene` 纯 props 内核（`SymmetryViewSceneProps`：group/symmetryType?/dark 主题解耦（#0a0a1a↔#f4f4f7，主画布传主主题、窗口传窗口主题）/variant dual-solid（cube↔octahedron、icosahedron↔dodecahedron）/showAction/actionElementId/rotateSpeed/showFigureTitle/locked/onHint/hintOnIdle/replaySignal）+ `SymmetryView` context 壳（签名不变，GroupCanvas/旧浮动窗零改动）；`GroupSymmetryContext` 增 `symmetryVariant/setSymmetryVariant`（换群复位），场景内悬浮变体切换按钮移除改宿主入口（主画布 ViewPanel Shape 选项 / 窗口 ⚙ 面板）；`SymmetryViewParams`（variant/showAction/rotateSpeed 0.2-5/actionElementId/showFigureTitle）+ schema 入 ViewParams union；**ViewWindow 接入**：renderContent symmetry 分支（SymmetryViewScene 直渲：dark=窗口主题、showFigureTitle 缺省 false 防「双群名」、locked=config.locked、hintOnIdle=false 引导写在面板、onHint→内容区底部 ⟳ Replay/✕ Reset 唯一浮条）+ 参数面板 Symmetry View 段（unsupported 提示 supported 清单 / dual solid 切换（仅 cube|icosahedron 类群）/ Show element actions + Speed slider / Action element 列表点行触发-再点重播-✕ Reset pose / config.actionLocked 固定演示模式——列表与 Reset 隐藏只读 fixed 元素、只能 ⟳ Replay / Show figure title 复选）+ 视作自管理布局禁 ct 平移缩放（同 3d）；**replaySignal 显式重放**修复「动画播完无入口重看」缺口。TestPage 补 10 窗矩阵（A₄ 默认+演示 3-cycle / S₄ 对偶八面体 / D₆ 六边形 / C₈ 八边形 / V₄ 长方形 / A₅ 二十面体 / Q₈ unsupported / C₈ 受控 / D₆ actionLocked 固定演示）+ TestPage2 第 7 节「A₄ 在正四面体上的作用」博客配图。测试：dom `SymmetryWindowParams.component.test.tsx`(10) → 75 文件 1640 tests 全绿（node 56/1488 + dom 19/152）+ lint/typecheck/build + E2E 13 passed。版本 1.22.0→1.23.0（package.json/package-lock/welcome.version zh/en）。余 tree/prestable + action 窗口内 sylow/coset（延后）

- ✅ **FGVE 阶段 3 批次八（v1.24.0，2026-09-07，打包最小闭环：`@groupviz/core` + `@groupviz/react` 双包可构建 + `examples/host-minimal` 最小宿主消费冒烟）**：**core 包** `vite.pkg.core.config.ts` library mode 出 `dist-pkg/@groupviz/core`（566KB 唯一外部依赖 zod）；**react 包** `vite.pkg.react.config.ts` 自 `src/package/react.ts` 门面出包（首批 SetView + CycleView + I18n/Theme Provider + renderTex，19.6KB 不内嵌 core）；**coreToPackage transform 钩子**（scripts/pkg/config-common.ts）把 121 源文件相对 `../../core` 导入改写为 `@groupviz/core` external——**rolldown/vite8 相对路径不走 resolveId 的实测坑，须用 transform**；`scripts/pkg/finalize-pkg.mjs` d.ts 树分发 + 相对 core 引用→包名重写 + 双包 package.json + **theme.css 从 src/index.css 自动提取**（dark/light 134 变量 + 视图基础类，防双源漂移）；`tsconfig.pkg.json` emitDeclarationOnly；npm `build:pkg`；dist-pkg 入 .gitignore 不入库。**host-minimal**（examples/）真实 npm 消费（alias 正则完整匹配防前缀吞子路径 + `fs.allow` 放行仓库根修 KaTeX 字体 403）；playwright 浏览器实证：S₄ 24 元素 Set 网格 / Cycle 中心 e 辐射 / 切 C₆ / 导出 SVG 21.5KB / console 零 error。待办：CI 冒烟接入、视图全量入包（8 个带 i18n/context Scene 按文案 props 化策略重构）、exports 字段、descriptor 真 round-trip 载群。版本 1.23.0→1.24.0
- ✅ **FGVE 阶段 3 批次九（v1.25.0，2026-09-07，首批带 i18n Scene 入包：语言包入包策略 + CayleyView/CosetStripScene 双 Scene 直达）**：**策略换道（用户拍板，覆盖批次八「文案 props 化」计划）**——Scene 保留 `useTranslation`，react 包**内置语言包**（bundled `translations.ts` 1385 行 zh+en）并导出 `I18nProvider`；消费端须用**包内** `<I18nProvider>` 包裹 Scene（模块重复 → 与主应用 Provider 各为独立实例，不包则 t() 回落原始 key）；**CosetStripScene 拆壳**——CosetStripView.tsx 原 Scene+壳混合（Scene 用 useTranslation 3 key、壳读 useGroup/useHover），拆新文件 `CosetStripScene.tsx` 纯 render 内核，CosetStripView 降级 context 壳（签名不变、FloatingViewWindow import 改指新文件）；CayleyView 本就零 context 依赖直入包；**react 门面扩展**（src/package/react.ts：+CayleyView/CayleyViewProps + CosetStripScene/CosetStripSceneProps + I18nProvider）；**d.ts 修复**——I18nContextValue 加 export（tsc d.ts emit TS4058）；**构建产物**：core 566KB（唯一依赖 zod）、react 19.6KB→103KB（语言包入包）；finalize-pkg .dts 清理 try/catch 容错（sandbox 批量删 >50 fail-closed，改 find -depth per-file）；TestPagePkgConsume 批次九区块整体包内 I18nProvider + CayleyView 块 + CosetStripScene 块（useS3CosetMap 按非单位自逆元素作 H 算 3 右陪集彩色条带）；Playwright 实测 3 条带 + 中文图注「子群凯莱图 ⟨H⟩」+ console 零错误。门禁：typecheck/lint/vitest 1640（75 文件）/build:pkg/主应用 build 全绿。版本 1.24.0→1.25.0。余 6 Scene（table/3d/sublattice/homomorphism/action/symmetry）按同策略延后
- ✅ **FGVE 阶段 3 批次十（v1.26.0，2026-09-07，余 6 Scene 全量入包收官视图入包阶段）**：**入场三档**——🟢 直入 `TableView`（TableViewFromContext 已是独立壳，TableView.tsx 纯 props 内核直入；触发唯一改造：`triggerDownload` 由 `utils/export.ts` 抽零依赖模块 `src/utils/download.ts` 切断 gifenc 链路（顶层 import 整 gifenc 库被保守内嵌 +24KB），export.ts re-export 保主应用兼容，`tsconfig.pkg.json` include 补 `src/types` 让 gifenc.d.ts 入 program 治本）+ `ActionScene`（独立文件、零 context 依赖直入）；🟡 拆文件 `HomomorphismScene`（Scene + HOMO_COLORS/KERNEL_RED/IMAGE_CYAN/bezierPath/findGeneratorElements 整体迁出 HomomorphismScene.tsx，壳 HomomorphismView 留 useGroup 装配 + FirstIsomorphismAnimation 注入 theoremAnimation；无主题依赖）+ `SymmetryViewScene`（Scene + 全部几何 helpers getCyclicFigure/DihedralFigure/Cube/Icosahedron/Octahedron/Dodecahedron/Tetrahedron/Rectangle + SymmetryScene/AnimatedGeo/AxisMarker/UnsupportedOverlay/useAnimatedRotation 全渲染链 800 行迁出 SymmetryViewScene.tsx，壳留 useGroup/useTheme 喂 dark={theme==='dark'}；原本就纯 props 仅 dark prop 决定场景配色）；🔴 拆文件 + theme props 化 `Cayley3DScene`（NodeSphere 原 useTheme 决定 label 阴影改新 `isDark: boolean` prop，Scene 背景色与 NodeSphere 阴影同源 effTheme = `themeProp ?? useTheme().theme`；`Cayley3DSceneProps` 加 `theme?: 'dark'\|'light'`，缺省回落 ThemeContext 行为零回归）+ `SublatticeScene`（Scene 主体 `buildPalette(theme)` 与 SeriesPanel chips 同源 effTheme；SeriesPanel 加可选 `theme?` prop 保持内部组件签名兼容）。**core 索引补 `export * from './symmetryType'`**——SymmetryViewScene 引入 getSymmetryType 经 coreToPackage 改写后主应用 build MISSING_EXPORT，补 export + 重打 core 包；**d.ts 修复**——`ThemeContext.tsx` `interface ThemeContextValue` 改 `export interface ThemeContextValue`（Cayley3D/Sublattice 首次拉 useTheme 进 react pkg program，TS4058 与批次九 I18nContextValue 同解法）。**react 门面**（src/package/react.ts）+ 6 Scene（HomomorphismScene + SymmetryViewScene + Cayley3DScene + SublatticeScene + TableView + ActionScene，原 4 → 10 Scene）。**消费者 import 同步**——FloatingViewWindow 4 处 import 拆分（Cayley3D/Homomorphism/Symmetry/Sublattice 各 Scene 与 View 分离）+ 测试文件 import 路径同步。**TestPagePkgConsume 扩展**（主应用懒加载区块）——6 新消费块（Table/Action/Homo C₆→C₂ mod2 满同态纯 multiply 构造/Sublattice/Cayley3D theme="dark" 显式与容器解耦/Symmetry D₄ dark + showFigureTitle={false} 避双群名）。**Playwright 实证**（`vite preview localhost:4174`）——10 区块 present（pkg-set/cycle/cayley/cosetstrip/table/action/homo/sublattice/cayley3d/symmetry）/4 SVG + 2 R3F canvas/6 个 homo source 节点 = C₆ 阶验证双群映射/console 零 error/视觉 S₃ conjugation 环 + 中心 e ★ + C₆→C₂ 满同态紫绿映射 + D₄ dihedral 长方体三轨道变体。**门禁**——typecheck/lint/vitest 75 文件 1642 tests 全绿/build:pkg（react 103→231KB gzip 48→63KB，gifenc 0 出现）/主应用 build 全绿。**FGVE 阶段 3 视图入包阶段收官**：10/13 视图入包（set/cayley/cycle/table/3d/sublattice/homomorphism/cosetstrip/action/symmetry）。**延后项** = tree/prestable props 化（v1.17.0 起持续延后）+ action 窗口内 sylow/coset actionKind（v1.22.0 起持续延后）。**阶段 3 余待办** = CI Playwright 冒烟接入 + `exports` 字段子路径 + descriptor 真 round-trip 载群。版本 1.25.0→1.26.0（package.json/package-lock/welcome.version zh+en 双语）。
...
- **先明确用户需求再动手**：任务范围不明确、或依赖已丢失的历史信息（如被删除/从未提交的文件、跨会话遗留状态）时，必须先向用户确认需求与内容，不得凭猜测自行假设、擅自重建——参考 grilling 技能设计树的追问流程
...
- 收尾：开发/测试完成后，关闭所有启用的端口（如 vite :5173、后端 :8000）、以及本次会话启动的新程序/应用/进程

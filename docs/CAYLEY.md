...
| `torusHex` | S₄（星形对换 (12)(13)(14)） | **环面全六边形镶嵌**：24 顶点 / 36 边 / 12 个六边形面（= ⟨(12),(13)⟩、⟨(13),(14)⟩、⟨(12),(14)⟩ 三个 S₃ 子群的 12 个右陪集，每个六边形的边严格交替两个生成元），每顶点属 3 面、每边属 2 面 ⇒ χ = 24−36+12 = 0。周期格是 **60° 菱形** ⟨(3√3,3),(0,6)⟩（两向量等长 6）：S₄ **不存在矩形（无剪切）基本域**（水平周期只能 6√3·k、竖直只能 6m，凑指标 12 需 (k,m)=(6,1) 而 (0,3)∉Λ），所以粘合必然斜着绕、六边形在环面上是仿射正六边形。渲染配套：半透明环面壳 + 12 片贴曲面六边形面片（重心细分）+ 沿曲面采样的弧边（`SurfaceArcEdge`）+ 悬停/选中时点亮所属 3 片。R/r = 1.5（孔洞可见度 ↔ 六边形规整度折中）。**注意**：该形状与"6 六边形 × 2 行"那类无剪切环面镶嵌**不同构**（那张图 girth 4、含 24 个四环，星图 girth 6；穷举 S₄ 全部 2928 组三元生成子集均无命中） |
| `wordLengthSphere` | S₄/S₅（one-line 置换群，n≥4） | **字长球**（相邻对换生成集 (12)(23)(34)(45)，硬编码层配置 HARDCODED_LAYERS：S₄ `[1,3,5,6,5,3,1]` / S₅ `[1,4,9,15,20,22,20,15,9,4,1]`，与 BFS 分布核对）。**S₄ 分层球面重排 + 极点三方对称**：借截角八面体骨架（= 凯莱图）顶点周向角重排到字长层（ρ_k=R·sin(πk/K)、y_k=R·cos(πk/K)，顶点全贴球面）；第 1 / 第 K−1 层（e 与 w₀ 的三邻居）周向角重排成 **120° 三等分星形**（相位取挪动最小以保住骨架结构），再整球绕 y 轴刚性旋转使默认视角下两极三脚架呈对称扇形。3D 零交叉、默认视角投影交叉 ≤16。**S₅ 字长球（纬度分层 + 正根胞格向量 + 边距松弛）**——① **纬度层**：y_l = R·cos(πl/10)（e 与 w₀ 在两极），第 l 层球截面半径 r_l = R·sin(πl/10)；小层（层 1/9 的 4 点）直接贴球面圆环，大层按投影模长排名取 r_l·√((rank+0.5)/n) 铺满圆盘（盘心→盘缘都有点，实心球）；② **只有层间有边**：相邻对换使字长恒 ±1 ⇒ 240 条凯莱边全部跨相邻层、层内无边；③ **正根胞格向量初值**：A₄ 的 10 个正根（= 10 个值对）手工投影到 2D——四个单根 (1 2)(2 3)(3 4)(4 5) 恰好落在 +x / +y / −x / −y（**四个生成元 = 四个方位瓣**），其余六个正根落在中间方位且模长更小；每个置换的投影 = 其**逆序集**上这些向量的和 ⇒ 父节点与子节点的投影方向天然相邻、边近似沿经线从上往下流动；④ **边距松弛**（320 轮）：同层节点斥力（作用半径 ≈ 1.9·r_l/√n，顶到球面边界为止）+ **边-边斥力**（AABB 预筛 + 每条边取 3 采样点做线段-线段最近距离）+ 角度弱锚定（拉回初值方位，保持四瓣/偶极场结构）+ y 弹簧（拉回名义纬度）；硬约束 y ∈ 纬度带 ±0.32×层间距、水平半径 ≤ min(r_l, √(R²−y²))；收尾把 r > 0.9·r_l 的节点**精确吸附到球面**（外圈贴壳、内部保留）。**S₅ 相邻对换凯莱图非平面**（二部图要求面数 F ≤ E/2 = 120，而欧拉公式 V−E+F=2 要求 F = 122，矛盾），固定纬度面 + 直线边必有极个别交叠——松弛把边在 3D 中真正错开。量化（R=15，\|G\|=120）：\|p\| ≤ R、贴球面 37 点、边长度中位 5.9 / 最长 9.9、边竖直度中位 0.67、节点最小间距 2.40、非共端点边对最近距离 > 0.15（近距对 < 0.55 仅 1 对）、布局耗时 ≈ 110 ms（确定性，无随机）。渲染配套：字长色阶（`wordLengthColor` 205°→330°）、深度雾（远处节点/边向背景淡化）、**半透明球壳**（半径 = 最外层节点所在球面；`wordLengthSphere` 专用，`depthWrite:false` 不遮挡球内节点，仅给「整个球」一个轮廓参照）。字长球下解除凯莱边大群限流（S₅ 480 push/240 边）并豁免大群节点抽稀 |

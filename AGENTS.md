...
| [docs/TESTING.md](docs/TESTING.md) | 测试体系：58 文件 1481 tests（node+dom 双项目）、Playwright E2E 13 tests、vitest 配置、覆盖率、测试约定 |
...
GroupViz/
├── src/
│   ├── __tests__/              # 58 个测试文件（1481 tests，node+dom 双项目），见 docs/TESTING.md
│   ├── components/
...
- ✅ **FGVE 阶段 2 批次一（v1.17.0，2026-08-28，set/cayley 视图 props 化 + 受控 ViewWindow）**：**GroupDescriptor v1 序列化协议**（src/core/descriptor.ts：zod schema GroupDescriptorSchemaV1{schemaVersion/symbol/name/order/elements[{id,label,order}]/multiply/properties/construction/source[local|import|gap|product]} + serializeDescriptor/deserializeDescriptor/descriptorToSymbol）+ **ViewConfig JSON 化**（src/core/types/viewConfig.ts：ViewWindowConfig{locked,zoomLocked,showInfo,viewportFixed,resizable}、SetViewParams{nodeRadius 8-120,gap 0-100,columns 0-50,showLabels}、CayleyActionParam{elementId,enabled,color}、CayleyViewParams{shape2D,multiplyType,actions,nodeRadius,showLabels}、ViewWindowGeometry、ViewWindowPersistData 全 zod schema，actions 上限 240，core 零 react/localStorage）；CAYLEY_SHAPES_2D 常量 + CayleyShape2D 类型置于 types/view.ts。**受控 CayleyView**（src/components/Canvas/CayleyView.tsx 纯 props：computeShape2DPositions 布局 null 回退 cayleyCircleLayout、normalizeCayleyActions 作用边、右/左乘、有向 marker 每实例唯一 cv{n} 前缀防多窗口冲突、自环、半直积 rewiring φ-不动点青色高亮、大群 culling、拖拽位置会话态形状/群变重置）。**SetView 受控化**（props 化 + SetViewFromContext adapter 壳保持主应用零变化）。**受控 ViewWindow**（FloatingViewWindow.tsx 中 724 行起新组件，旧 FloatingViewWindow 保留供主应用多视图）：通用参数面板（View Config 4 复选 + Set/Cayley 视图参数 + ↺ Reset）+ 受控/非受控双模式 + gv-vw- 前缀 versioned 持久化（按视图 schema 校验回退）+ 窗口拖/8 方向缩放 + 内容 Ctrl/Cmd 滚轮缩放（rAF 节流/双击复位）+ 参数面板外置右侧溢出翻左 + VIEWWINDOW_RESET_EVENT/resetAllViewWindows()（utils/resetViewWindows.ts）+ `/?test=1` TestPage 参数矩阵（11 set + 5 cayley 窗口）。测试：dom 新增 CayleyView 11/CayleyWindowParams 8/ViewWindowParams 6、node 新增 descriptor 5/viewConfig 8（合计 +38 → 58 文件 1481 tests 全绿）+ lint/typecheck/build 通过 + Playwright 浏览器 DOM 实测（C₄ 4 节点/4 曲边/marker、D₄ dualRing 8/12 边、S₃ left 无向、C₁₂ spiral、set 窗口节点）

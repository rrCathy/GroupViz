...
| 几何作用 | 对称旋转作用于多面体顶点（6 种几何体，12/24/60 顶点；A₄→截角四面体、S₄→截角立方体、A₅→截角二十面体） |
| 自定义作用 | 元素间画箭头、绑定生成元，自动补全 + 同态校验 Φ: G → Sym(X)，违规精确定位 |
...
- **测试体系** — 27 个测试文件 511 tests（vitest）
---
...
## 🔮 路线图
- [x] 9 种视图模式（含同态视图、陪集条带）→ 现 10 种（+轨道视图）
- [x] 多视图浮动窗口
- [x] 子群格（Hasse 图）
- [x] 对称性视图（多面体旋转动画）
- [x] 小群预计算注册表（阶 1–15）

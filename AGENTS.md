...
- 🔄 **当前进行**:FGVE 引擎化收尾(双包已发布,余 tree/prestable props 化、action 窗口 sylow/coset、CI 消费冒烟接入、exports 子路径 —— 见 [docs/ROADMAP.md](docs/ROADMAP.md) §2)。
- ✅ **已完成**:近期功能夯实期(2026-08-23 收官)+ FGVE 阶段 2 视图 props 化(10/13 视图受控 ViewWindow)+ 阶段 3 双包 @groupviz/core / @groupviz/react **v2.0.0 npm 发布**(2026-09-09,发布门禁 publish:smoke)+ **v2.1.0 消费端卡点优化**(2026-09-10:元素引用按 label/id 解析、`useSceneState`/`SceneThemeRoot`/`SceneHoverBubble`、`theme` 统一、阈值可覆盖、`docs/API.md` 入包、i18n 免 Provider 兜底)。全量逐次记录见 [docs/CHANGELOG.md](docs/CHANGELOG.md)。

...
**GroupViz** 是一个用于可视化与探索有限群论的交互式Web应用。提供 11 种视图模式、支持多种典型群族与群构建系统（直积/半直积/自同构群/商群/同态），所有数学公式均通过 KaTeX 渲染。
<p align="center">
...
| 自定义作用 | 元素间画箭头、点击/拖放绑定生成元，自动补全 + 同态校验 Φ: G → Sym(X)，违规精确定位 |
...
- **测试体系** — 28 个测试文件 542 tests（vitest）
---

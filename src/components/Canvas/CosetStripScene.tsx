...
// FGVE 打包期随视图层入包：仅依赖 i18n context（文案 key 由 @groupviz/react 自带
// I18nProvider/语言包供给），不读 useGroup/useHover/useTheme——context 组装壳
// 在 CosetStripView.tsx（主应用宿主专用，不入包）。
        ...
        {cosetStripData && cosetStripData.strips.length > 0 && cosetStripData.strips.map((strip: CosetStripInfo, si: number) => (
          <g key={`coset-bg-${si}`} data-coset-strip={si}>
            <rect
              ...
              stroke={strip.color + (strip.isSubgroup ? '55' : '28')}

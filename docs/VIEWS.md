...
- **正规子群 N 单独画一份凯莱图**，放在画布右侧的**可收起、可拖动的悬浮窗**（`components/Canvas/QuotientSubgroupInset.tsx`），箭头从**恒等陪集节点（= N 本身）**指过去；窗内小凯莱图的边 = **N 自己的最小生成元**（`findMinimalGenerators`，用户：「只需要展示它自己的生成元作为边就够了」）；收起 = 标题栏按钮缩成小药丸（`N |N|=n ▸`），点药丸原地展开；窗体以**屏幕像素**设计（经 `scale(k)` 落进 SVG；`k` 取 **CTM 的实际等比缩放**倒数——`preserveAspectRatio="xMidYMid meet"` 下 `px/单位 = min(rectW/vbW, rectH/vbH)`，不能用 `viewBox宽/容器宽`，且屏幕 px 位置要加 `origin` 补偿内容居中留白，见 `core/viewBox.ts` 的 `computeInsetMetrics`）⇒ 不随画布 viewBox 缩放变小，**右下角手柄可拖拽缩放**（240×180 ~ 720×600，默认 360×300；小容器按容器再收），拖动/缩放位置仅会话内有效；窗够大时每个小圆点下方显示 KaTeX 元素标签（>12 个元素或节点过小则只留悬停 title）。数据字段 `cosetMemberLabels` / `cosetInternalLayout` / `cosetInternalEdges`（core 的 `computeQuotientGroup` 写入，各陪集共享同一份内部布局）；

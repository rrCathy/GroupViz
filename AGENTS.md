...
- ✅ 13 种视图模式：set / cayley / cycle / table / 3d / symmetry / sublattice / homomorphism / cosetstrip / action / sylow / tree / prestable。**其中 11 个已完成 props 化入包**（`@groupviz/react` 收录 11 个 Scene，2026-09-16 sylow 收尾）；tree / prestable 两个群展示专用视图（入口在左侧「群展示」面板底部，不在视图面板卡片中）与**无限群**方向绑定、当前无消费需求，**不再 props 化**，移交拓展包轨道（见 [docs/PLAN_EXTENSION_PACKS.md](docs/PLAN_EXTENSION_PACKS.md) §9）

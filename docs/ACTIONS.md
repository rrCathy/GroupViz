...
| `geometry` | 几何对称作用 | 多面体顶点（6 种，12/24/60 顶点） | 生成元旋转 + 顶点最近邻匹配，BFS 扩展（见 §3） |
| `custom` | 用户自定义 | 用户指定大小 \|X\|（1..20） | 箭头 + 补全 + BFS 扩展（见 §4） |

共轭作用中：轨道 = 共轭类、固定点 = 中心 Z(G)、Stab(x) = 中心化子 C_G(x)。（共轭特化标注为后续待办。）

## 3. 几何作用算法（BFS 构造）

不逐元素独立旋转——`elementRotation.ts` 的哈希轴分配不保乘法表（Φ(g·a) 的哈希轴与 R(g)R(a) 不一致，同构被破坏）。改为：

1. 仅对**生成元**做 `computeElementRotation` → Rodrigues 旋转矩阵 → 旋转全部顶点 → 最近邻匹配得生成元置换
2. 生成元置换非双射（`new Set(p).size !== n`）→ 失败（`badGenerator`，如 A₅ 部分 5-cycle 落在非真对称轴）
3. 双射则 BFS 从单位元扩展：Φ(el·gen) = Φ(el)∘Φ(gen)，**天然满足乘法表，同态自动合法**

**匹配建议**：A₄→截角四面体、S₄→截角立方体、A₅→截角二十面体。其他群组合可能平凡作用（rotation=null → 恒等置换，诚实合法）。

**已知限制**：A₅ + 截角二十面体/十二面体部分生成元会失败（5 阶元哈希轴含 3 条坐标面轴非真轴）→ 校验失败红字警告，不做轴修正。

## 4. 自定义作用（箭头 + 补全 + 校验）

用户交互：点击元素 A 再点击 B → 未绑定箭头 A→B；拖拽/点击生成元 chip 选中 → 点击未绑定箭头绑定；点击已绑定箭头解绑。
...
rotationMatrix(axis, angle)（Rodrigues）/ applyRotationMatrix
computeConjugationPerms(group)
getGeometryVertices(geometry)
computeGeometryPerms(group, geometry) → GeometryPermResult{ perms, ok, badGenerator? }
...
**类型**（`src/core/types.ts`）：`GroupActionKind`('conjugation'|'geometry'|'custom')、`GroupActionArrow`{generatorId|null, from, to}、`OrbitInfo`{representative, elements}、`GroupActionComputation`{n, perms: Map<elId, number[]>, orbits, orbitOf, stabilizers, isHomomorphism, violation?}、`GroupActionDef`{kind, geometry?, setSize?}。

**状态**（`src/context/actions/GroupActionContext.tsx`）：actionKind/actionGeometry/actionSetSize/actionArrows/actionEditing/actionComputation/actionError/actionSelectedElement/actionHoverElement/actionShowEdges；动作 createConjugationAction/createGeometryAction/startCustomAction/addArrow/bindArrow/removeArrow/clearArrows/completeCustomAction/setActionSelectedElement/setActionHoverElement/setActionShowEdges/clearAction。群切换时 prevGroupRef 重置（queueMicrotask）。

## 6. 视图（轨道视图，`src/components/Canvas/ActionView.tsx`）

- 轨道簇布局：按大小升序左→右（固定点 ★ 簇最左，呼应书中「稳定元在左」）；簇内环形排列，半径 max(26, size·5)
...
- 显示模式元素用 TeX 标签（共轭作用，元素=群元素）或数字 1..n（几何/自定义）
- 自定义编辑模式：|X| 元素围圈、生成元 chips 行（可拖拽/点击）、虚线未绑定箭头、红字错误提示
- 视图 2000×2000 viewBox，随 GroupCanvas 平移缩放；FloatingViewWindow 中包 SvgPanZoom
- isTooLarge 阈值 120（与 symmetry/sublattice 同级）

## 7. 面板

- **左侧**（GroupActionPanel，HomomorphismPanel 之后）：三个来源按钮 + 几何体下拉（6 种 + 匹配建议）+ |X| 输入（1..20）+ 编辑模式箭头列表/错误/完成并验证/清除
